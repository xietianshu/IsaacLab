...
def configure_x11(statefile):
    ...
    if __ISAACLAB_TMP_XAUTH == "null" or not Path(__ISAACLAB_TMP_XAUTH).exists():
        __ISAACLAB_TMP_XAUTH = Path(subprocess.run(["mktemp", "--suffix=.xauth"], capture_output=True, text=True).stdout.strip())
        statefile.set_variable("__ISAACLAB_TMP_XAUTH", str(__ISAACLAB_TMP_XAUTH))
        xauth_cookie = subprocess.run(["xauth", "nlist", os.environ['DISPLAY']], capture_output=True, text=True).stdout.replace('ffff', '')
...
def x11_check(statefile):
    ...
    if __ISAACLAB_X11_FORWARDING_ENABLED == "null":
        ...
        print("[INFO] It will fail if there is no display, or this script is being run via ssh without proper configuration.")
