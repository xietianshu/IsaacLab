...
def check_docker_version_compatible():
    ...
    if int(docker_version.split('.')[0]) >= 25:
        print(f"[ERROR]: Docker version {docker_version} is not compatible with Apptainer version {apptainer_version}. Docker version must be 25.x.x or lower. Exiting.")
        sys.exit(1)
    else:
...
def check_singularity_image_exists(container_interface):
    ...
    result = subprocess.run(["ssh", CLUSTER_LOGIN, f"[ -f {CLUSTER_SIF_PATH}/{container_interface.image_name}.tar ]"], capture_output=True, text=True)
    if result.returncode != 0:
        print(f"[Error] The '{container_interface.image_name}' image does not exist on the remote host {CLUSTER_LOGIN}!", file=sys.stderr)
        sys.exit(1)